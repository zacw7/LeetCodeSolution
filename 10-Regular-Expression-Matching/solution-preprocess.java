class Solution {
    // DFS
    // optimize: proprocess the pattern string
    public boolean isMatch(String s, String p) {
        ...
        for (int i = 0; i < sb.length() - 3; i++) {
            if (sb.charAt(i + 1) == '*') {
                while (i + 3 < sb.length() && sb.charAt(i + 3) == '*'
                        && (sb.charAt(i) == '.' || sb.charAt(i) == sb.charAt(i + 2))) {
                    sb.deleteCharAt(i + 3);
                    sb.deleteCharAt(i + 2);
                }
            }
        }
        return matchHelper(s.toCharArray(), 0, sb.toString().toCharArray(), 0);
    }
